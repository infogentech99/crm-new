...
const COLORS = ['#0088FE', '#00C49F', '#FFBB28']; // Pending, In Progress, Completed
  ...
  if (isError) {
    return (
      <div className="text-red-500 text-center bg-white p-6 rounded-lg shadow-md h-80">
        Error loading task status data: {error?.message || 'Unknown error'}
      </div>
    );
  }

  const summaryData = data?.taskStatusSummary || {};
